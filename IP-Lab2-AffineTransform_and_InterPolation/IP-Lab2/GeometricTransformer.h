...
public:
	...
	*/
	virtual uchar Interpolate(
		float tx, float ty, uchar* pSrc, int srcWidthStep, int nChannels, int i) = 0;
	PixelInterpolate();
	~PixelInterpolate();
};
...
{
public:
	uchar Interpolate(float tx, float ty, uchar* pSrc, int srcWidthStep, int nChannels, int i);
	BilinearInterpolate();
	~BilinearInterpolate();
};
